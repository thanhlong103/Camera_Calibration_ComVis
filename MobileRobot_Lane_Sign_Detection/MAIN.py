...
if __name__ == '__main__':
    ...
    while True:
        ...
        if range1[0] <= top < range1[1]: # Move forward with error for PID value: eror = 0
            arduino.write(b'a')
            print('a')
        elif range2[0] <= top < range1[0]: # Move forward with error for PID value: eror = -2
            arduino.write(b'b')
            print('b')
        elif range3[0] < top < range2[0]: # Move forward with error for PID value: eror = -4
            arduino.write(b'c')
            print('c')
        elif range1[1] <= top < range2[1]: # Move forward with error for PID value: eror = 2
            arduino.write(b'd')
            print('d')
        elif range2[1] <= top < range3[1]: # Move forward with error for PID value: eror = 4
            arduino.write(b'e')
            print('e')
        elif top >= range3[1]: # Turn right to return to the lane
            arduino.write(b'f')
            print('f')
        elif top >= range3[1] and bottom >= range3[1]: # Turn left to return to the lane
            arduino.write(b'g')
            print('g')
        elif top <= range3[0]: # Turn left to return to the lane
            arduino.write(b'h')
            print('h')
        elif top <= range3[0] and bottom <= range3[0]: # Turn right to return to the lane
            arduino.write(b'i')
            print('i')